...
func Transcode(params *TranscodeInput) (metadata.Metadata, error) {
	...
	for _, resolution := range params.Resolutions {
		...
		go func() {
			...
			rendetion, err := createMP4Rendetion(
				params.InFile,
				rendetionOutFile,
				sourceMetadata,
				template)
			...
			hlsRendetion, err := createHLSSegments(rendetion.Outputs.MP4.Path, hlsOutDir, resolution)
			if err != nil {
	...
	go func() {
		...
		thumbnail, err := createDefaultThumbnail(params.InFile, thumbnailOutFile, seektime)
		if err != nil {
	...
	masterPlaylist, err := createHSLMasterPlaylist(masterPlaylistOutFile, meta.Rendetions)
	if err != nil {
		return meta, err
	}
...
func createMP4Rendetion(src, dest string, meta metadata.SourceMetadata, template Template) (metadata.Rendetion, error) {
	cmd := exec.Command(
		"ffmpeg",
		"-y",
		"-i",
